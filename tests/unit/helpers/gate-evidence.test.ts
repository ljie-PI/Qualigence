...
describe("Gate evidence verifier", () => {
  ...
  it("provisions ripgrep and workspace build before hosted Ticket 33 Gate tests", async () => {
    ...
    expect(readiness).toContain('join(COMPOSE_DIR, ".e2e-runtime-")');
    expect(readiness).toContain("${composePath(REPO_ROOT)}:/workspace:ro");
    expect(readiness).toContain("./${ctx.runtimeDirName}/bootstrap.mjs:/bootstrap.mjs:ro");
    expect(readiness).toContain('entrypoint: ["node", "/bootstrap.mjs"]');
    expect(readiness).toContain("./${ctx.runtimeDirName}/jwks-server.mjs:/jwks-server.mjs:ro");
    expect(readiness).toContain("node /jwks-server.mjs");
    expect(readiness).not.toContain("--volume");
    expect(readiness).not.toContain("/harness/bootstrap.mjs");
    expect(readiness).not.toContain("/harness");
    expect(readiness).not.toContain("workspaceHarnessFile");
    ...
    expect(harness).toContain('join(COMPOSE_DIR, ".e2e-runtime-")');
    expect(harness).toContain("${composePath(REPO_ROOT)}:/workspace:ro");
    expect(harness).toContain("./${ctx.runtimeDirName}/bootstrap.mjs:/bootstrap.mjs:ro");
    expect(harness).toContain('entrypoint: ["node", "/bootstrap.mjs"]');
    expect(harness).toContain("./${ctx.runtimeDirName}/jwks-server.mjs:/jwks-server.mjs:ro");
    expect(harness).toContain("node /jwks-server.mjs");
    expect(harness).toContain("./${ctx.runtimeDirName}/seed-ls11-product-surface.mjs:/seed-ls11-product-surface.mjs:ro");
    expect(harness).toContain("./${ctx.runtimeDirName}/verify-ls11-external-artifacts.mjs:/verify-ls11-external-artifacts.mjs:ro");
    expect(harness).toContain('"/seed-ls11-product-surface.mjs"');
    expect(harness).toContain('"/verify-ls11-external-artifacts.mjs"');
    expect(harness).not.toContain("--volume");
    expect(harness).not.toContain("/harness/bootstrap.mjs");
    expect(harness).not.toContain("/harness");
    expect(harness).not.toContain("workspaceHarnessFile");
