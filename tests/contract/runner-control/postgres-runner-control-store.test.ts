...
describe("PostgresRunnerControlStore persisted policy migration", () => {
  it("rejects every projectless persisted Job on read and renewal without changing expiry", async () => {
    ...
    try {
      await runtime.withTenant(TENANT_ID, async ({ db }) => {
        await db.insertInto("execution_leases").values({
          tenant_id: TENANT_ID, run_id: "run-projectless", job_id: "job-projectless", runner_id: "runner-1", session_id: "session-1",
        ...
        await expect(db.selectFrom("execution_leases").select("expires_at").where("run_id", "=", "run-projectless").executeTakeFirstOrThrow()).resolves.toMatchObject({ expires_at: expiresAt });
      });
    } finally {
      await runtime.close();
    }
  ...
  it.each([
    ["malformed Job identity", { jobId: 12, projectId: "project-test" }],
