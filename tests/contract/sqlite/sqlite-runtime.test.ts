...
describe("SqliteRuntime", () => {
  it("opens with required pragmas and reopens at the supported schema version", async () => {
    ...
    expect(await first.schemaVersion()).toBe(SUPPORTED_SCHEMA_VERSION);
    await first.close();

    const second = await SqliteRuntime.open({ filename, busyTimeoutMs: 5_000 });
    expect(await second.schemaVersion()).toBe(SUPPORTED_SCHEMA_VERSION);
    await second.close();
  });
