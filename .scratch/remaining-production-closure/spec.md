...
The tracked ticket set is the current execution authority. Each ticket carries its own dependencies, migration allocation, contexts, expected touchpoints, hard exclusions, focused Gate, post-review acceptance, behavior matrix, status, and final evidence so no parallel plan or status ledger is required.

## Solution

Complete production closure through two dependency-ordered implementation lanes and a release convergence phase.

### Two-phase closure authority — 2026-08-27 maintainer decision

The remaining closure now proceeds in two explicit phases:

1. **Implementation and deterministic automated validation.** Tickets 32, 33, 34, and the implementation portion of 35 may be developed, reviewed, validated by their focused/deterministic automated Gates, and merged in their documented code order before human Windows or real-provider evidence exists. A ticket may not claim release, Graph v1 `frozen`, signed native acceptance, real-provider acceptance, or published release success in this phase.
2. **Integrated human/release validation.** After all code implementation tickets are merged and one exact release-candidate commit is selected, Ticket 31 runs the signed Windows local-console/RDP acceptance and Ticket 46 runs real-provider validation. They may run in parallel on that same candidate. Ticket 35 then validates their immutable evidence together with Ticket 33 CI/browser and Ticket 34 release inputs, emits the deterministic candidate/frozen decision, and is the only ticket that may close the production/release convergence.

For these remaining tickets, `Blocked by` distinguishes an **implementation dependency** from a **final release-resolution dependency**. The latter blocks final release/freeze claims, not implementation work. This is a maintainer-approved reassignment under the Testing Decisions external/manual-infrastructure exception: no synthetic substitute, skipped test, fabricated artifact, altered signature, or weakened security invariant is permitted. Normal per-ticket focused Gates, typecheck, diff checks, complete-matrix review, and every available automated acceptance still run before an implementation PR; only the explicitly named human, credentialed-provider, cross-CI, or release-publication evidence waits for phase 2.
...
- The current code-first frontier begins with Ticket 32. Ticket 33 starts only after Ticket 32's implementation PR is merged because both own CI workflow/Gate composition; Ticket 34 starts only after Ticket 33's Gate/artifact contract is merged; Ticket 35's implementation starts only after Ticket 34's release-manifest implementation is merged. Ticket 31 and Ticket 46 may prepare in parallel throughout, but their actual human/provider executions wait for the final integration candidate and run in parallel before Ticket 35's final decision. Final release/freeze resolution remains blocked until all phase-2 evidence validates. The per-ticket staged dependency declarations are authoritative.
- Each Task lists expected touchpoints rather than an exhaustive edit whitelist. Existing issue sections named `Allowed Files` are interpreted as **Expected Touchpoints** unless a path or category is explicitly listed as a hard exclusion. Agents may edit directly affected callers, adapters, tests, fixtures, generated schema metadata, and ticket evidence when necessary to satisfy the Task, provided the change remains within the Task's requirements and existing architecture. Stop for maintainer approval only when a change crosses a hard exclusion, introduces or reassigns a migration allocation, adds or changes package/workspace dependencies, changes a public/persisted contract beyond the Task's behavior, creates a new architectural seam, changes security authority, alters unrelated product behavior, or implements another ticket's feature scope.
...
- Do not create a pull request while implementation, review fixes, or E2E are still in progress. After the implementation and complete-matrix review are clean, required E2E passes, and final status evidence is committed, create one non-draft pull request. Exception: a maintainer may explicitly reassign an E2E that requires external credentials, real provider/network access, signed manual evidence, or similarly unavailable infrastructure to a dedicated follow-up ticket; the current ticket must record that deferral and must not convert synthetic substitutes into release evidence. The PR head may differ from the reviewed code head only by a final documentation-only evidence commit. Verify the remote code/test diff equals the reviewed code/test diff, then merge directly after required remote checks. If code or tests change after review or E2E, rerun focused tests, complete-matrix review, and E2E before merge.
...
- After an exact-base `/code-review` reports no core Critical or Important findings, provision the required E2E environment and run the ticket's E2E Gate unless that Gate has been explicitly reassigned to a dedicated follow-up ticket by maintainer decision. If E2E failure requires a code change, rerun focused non-E2E tests and a fresh complete-matrix `/code-review`; rerun E2E only after that review is clean. A deferred E2E remains required for final release claims and must record stable blocker or pass evidence in its follow-up ticket.
...
- This Testing Decisions protocol supersedes shorter per-ticket execution-protocol wording where the two conflict, including older issue wording that calls expected touchpoint lists a complete edit scope. Explicit hard exclusions in an issue remain binding until amended.
...
- LS-09 closure tests interrupt and resume exploration from durable checkpoints and exercise seed Skills and recovery budgets. Real configured Reference Model Profile execution is required for release claims, but maintainer-approved centralized provider validation may collect that external-credential evidence in a dedicated ticket rather than every implementation ticket.
...
- Before pull-request creation, every implementation ticket runs its focused Gate, `corepack pnpm typecheck`, `git diff --check`, complete-matrix exact-base review, and required post-review E2E unless a maintainer explicitly defers that external/manual Gate to a dedicated tracking ticket. The pull request must contain exactly that reviewed head and diff, plus an optional documentation-only evidence commit.
