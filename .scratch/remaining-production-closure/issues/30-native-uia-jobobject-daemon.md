...
- Findings addressed in this fix commit: shutdown now removes session authority only after host Job termination succeeds; the native Windows host checks the `TerminateJobObject` return value before dropping Job/process tracking. Reset success now propagates reset-helper Job termination failure instead of reporting success, while preserving the target AppSession authority.
- Fix commit: `01abc56e4322a100923a703d28777bf045e04031`.
