...
- review3-fix: complete-matrix Standards review head `8b771308f1c9327fe23d2f8a3393ed758073f989` reported an Important core blocker that `server-volume-permissions` was not idempotent on retained, already-prepared durable volumes because it ran `install -d -m 0770`/`chmod` before `chown` while declaring only `CHOWN` over `cap_drop: ALL`; a normal `docker compose down`/`up` with retained volumes could fail with `chmod: Operation not permitted` before Server startup. Fix commit `20e0df89589d9aa6f2587bff4fbf4913e36eaebd` keeps the same hardened service boundary (no network, no secrets, no public ports, mounts only `artifactdata` and `skill_signing_data`, only `CHOWN` capability) but makes preparation idempotent by creating the mount points, temporarily chowning the directory roots back to `0:0`, applying mode `0770` while root owns those directory roots, and then recursively chowning the state back to uid/gid `1000:1000`; Server remains non-root/read-only and still depends on `service_completed_successfully`. README and secrets notes now document retained-volume idempotence without widening the secret or public-infra boundary. Component/E2E coverage now asserts the command order and runs the permission-prep command twice on the same retained fresh Docker volumes before the uid/gid `1000` read-only write check. Gates run for this review3 fix before the evidence update: `docker compose --env-file deployments/self-hosted/compose/.env.example -f deployments/self-hosted/compose/compose.yaml config --quiet` passed; `CI=true corepack pnpm vitest run tests/component/server` passed (1 file / 8 tests); `CI=true corepack pnpm vitest run tests/e2e/self-hosted/compose.test.ts` passed (1 file / 10 tests, including the two-pass permission probe); `CI=true corepack pnpm typecheck` passed; `git diff --check` passed. Targeted idempotence reproduction using the pinned Node runtime image, read-only root, no network, `--cap-drop ALL --cap-add CHOWN`, and retained named volumes passed: first and second prep runs both printed `1000:1000 770` for both state directories, then uid/gid `1000` wrote both probe files under a read-only runtime. Guard check `CI=true corepack pnpm vitest run tests/e2e/self-hosted/external-runner.test.ts` still fails closed with stable `ExternalRunnerUnavailable` because no real external Runner harness command is configured, so no final full-loop E2E/PR evidence is claimed. Ticket status remains `claimed`; fresh complete-matrix review is required at the new head before post-review full-loop E2E/PR work.
- review4-fix: complete-matrix Spec review head `9d207389542e177ba0afb517c6855e4dd4cce9e7` reported a Critical core blocker that the Compose Console service runtime image is built from `caddy:2.8-alpine`, but the Console healthcheck ran `node -e`, so the Caddy runtime lacked the command required to become healthy and the proxy/Compose readiness path could never pass. Fix commit `58c2e17a2c73d63f72f169cc18cd16148a995cc5` replaces only the Console healthcheck with `wget -qO- http://127.0.0.1:8080/ >/dev/null`, preserving the static Caddy Console image and avoiding any Node runtime dependency. Component and Compose E2E topology coverage now assert the Console healthcheck uses `wget`, does not require Node, remains tied to `console.Dockerfile`, and validates `command -v wget` inside the pinned Caddy runtime image under read-only, no-network, all-capabilities-dropped conditions. Gates run for this review4 fix before the evidence update: `docker compose --env-file deployments/self-hosted/compose/.env.example -f deployments/self-hosted/compose/compose.yaml config --quiet` passed; `CI=true corepack pnpm vitest run tests/component/server` passed (1 file / 9 tests); `CI=true corepack pnpm vitest run tests/e2e/self-hosted/compose.test.ts` passed (1 file / 11 tests); `CI=true corepack pnpm typecheck` passed; `git diff --check` passed. Targeted Caddy runtime command proof: `docker run --rm --read-only --cap-drop ALL --network none --entrypoint /bin/sh caddy:2.8-alpine@sha256:af32e97399febea808609119bb21544d0265c58a02836576e32a2d082c262c17 -ec 'command -v wget'` failed under Git Bash path conversion, then the equivalent `MSYS_NO_PATHCONV=1 ...` passed and printed `/usr/bin/wget`. Previous fixes are preserved: idempotent `server-volume-permissions`, PostgreSQL/MinIO private boundary, approved Runner gRPC `50555`, DockerUnavailable/ExternalRunnerUnavailable fail-closed behavior, and no migration/schema changes. Ticket status remains `claimed`; no final full-loop E2E/PR evidence is claimed.
