...
### review-fix 6 — 2026-08-25

- Reviewed head: `03f3ee81dd49bfbfa4d9caa144ca76e4ea5c5e78`.
- Core blockers fixed: screenshot masking now uses the union of page-known sensitive mask IDs from the initial DOM collection and the latest pre-screenshot recheck, including marked elements hidden during DOM collection, so a classified element made visible before screenshot is masked without broadening to unrelated equal pixels. Pre-dispatch permit rejection or abort after page epoch setup now ends the page epoch without retaining a record and removes the rejected marker/mask attributes from page-visible classified elements, leaving no dispatch side effect or stale causal marker.
- Fix commit: `b5d8d90c4bfb482529483e4d78b0b3013c4ffd19`.
- Gates run: `corepack pnpm exec tsc -b packages/target-adapters/web-playwright/tsconfig.json` (passed; refreshed local dist used by package self-tests); `CI=true corepack pnpm vitest run tests/component/web-execution/reflected-secret-evidence.test.ts` (passed: 1 file / 26 tests); `CI=true corepack pnpm vitest run tests/unit/target-adapters/web-playwright/browser-session.test.ts tests/component/web-execution/playwright-click.test.ts tests/component/web-execution/playwright-observation.test.ts tests/component/web-execution/reflected-secret-evidence.test.ts` (passed: 4 files / 83 tests); `CI=true corepack pnpm vitest run tests/e2e/web-execution/value-ref.test.ts` (passed: 1 file / 1 test); `CI=true corepack pnpm typecheck` (passed); `git diff --check` (passed).

### review-fix 7 — 2026-08-25

- Reviewed head: `1db89fee9abde5c21f2005adc66e1cf50c0bd1f0`.
