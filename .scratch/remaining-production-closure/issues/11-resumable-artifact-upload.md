...
- [x] Manifest binds tenant, project, Run, Artifact ID, size, SHA-256, media type, sensitivity, and fixed 256 KiB chunking.
- [x] Core returns missing ranges; chunks are offset-idempotent; ACK follows durable bytes+manifest verification.
- [x] Trace referencing unacknowledged Artifact is rejected or held without durable acknowledgement.
- [x] Lost owner may finish only previously registered manifests; restart/reconnect resumes from Spool safely.
...
- review-fix: reviewed head `f1e8d25ddf53be25aa6680ab3fba6d49b115555d`; fixed complete-matrix core blockers for Self-hosted tenant Runner application Artifact upload/Trace authority wiring, leased Run + authenticated tenant/project/runner chunk binding, lost-owner manifest/chunk boundary, reconnect Artifact-before-Trace replay, production Runner artifact spooling, and no-premature-ACK verification failure coverage. Fix commit: `cfde0cf` (`Fix ticket 11 artifact upload blockers`). Gates run clean before the fix commit: `CI=true corepack pnpm vitest run tests/conformance/runner-protocol tests/contract/runner-spool tests/contract/artifact-fs tests/contract/artifact-s3 tests/contract/postgres/artifact-upload.test.ts tests/unit/runner/trace-upload-pump.test.ts` (13 files / 148 tests), `CI=true corepack pnpm typecheck`, and `git diff --check`. No PR created and no resolved/final PR evidence added.
- review2-fix: reviewed head `8dd3826ac122324afbbbccc0398b49533d652b87`; fixed review2 core blockers for production run-unique/stable Artifact IDs and standalone Runner reconnect/restart recovery. `SpoolingArtifactObserver` now derives deterministic Run-bound Artifact IDs from the Run namespace, captured source name, and content hash; rewrites graph/node Trace evidence refs to those IDs; and keeps manifest/chunk/spool/upload storage on the same ID. The standalone Runner app now persists the rotating resume token encrypted in Runner Spool, enumerates pending Run IDs durably, and replays pending Artifact uploads before Trace after initial connect/reconnect without re-executing actions or inferring completion. Fix commit: `f67dc0b` (`Fix ticket 11 production replay blockers`). Gates run clean before the fix commit: `CI=true corepack pnpm vitest run tests/conformance/runner-protocol tests/contract/runner-spool tests/contract/artifact-fs tests/contract/artifact-s3 tests/contract/postgres/artifact-upload.test.ts tests/unit/runner/trace-upload-pump.test.ts` (13 files / 153 tests), `CI=true corepack pnpm typecheck`, and `git diff --check`. No PR created and no resolved/final PR evidence added.
- review3-fix: reviewed head `bb08b08f0cc00e15721162e1b7ae01e3749ae02a`; fixed review3 core blocker for lost-owner registered Artifact replay after reconnect/restart. Runner Spool now durably records server Artifact registration/progress acknowledgements, and `ArtifactUploadPump` resumes a previously registered manifest from that evidence through direct chunk upload without re-registering after lease loss. Manifests with no prior registration/progress evidence still attempt Core registration and fail closed after lease loss without uploading chunks or advancing Trace. Fix commit: `4c3adbba73163dd6d564539612df3cfff47bcc0d` (`Fix lost-owner artifact replay`). Gates run clean before the fix commit: `CI=true corepack pnpm vitest run tests/conformance/runner-protocol tests/contract/runner-spool tests/contract/artifact-fs tests/contract/artifact-s3 tests/contract/postgres/artifact-upload.test.ts tests/unit/runner/trace-upload-pump.test.ts` (13 files / 156 tests), `CI=true corepack pnpm typecheck`, and `git diff --check`. No PR created and no resolved/final PR evidence added.
- post-review-e2e: added `tests/e2e/self-hosted/artifact-upload.test.ts` for the required Ticket 11 acceptance. It exercises the tenant-bound Self-hosted Runner gRPC application with PostgreSQL upload/Trace state, encrypted Runner Spool recovery, and real MinIO-backed `S3ArtifactStore`: pre-ACK Trace reference is rejected without cursor advancement, manifest and first chunk remain unacknowledged, reconnect/replay uploads the missing chunk before Trace, the final ACK persists verified object bytes, and Trace is then accepted. Repository limitation recorded: `apps/server` Self-hosted composition still wires `LocalArtifactStore`; this acceptance uses the existing `TenantRunnerApplicationResolver` Artifact upload seam with the repo's S3 provider because S3 Server composition is reserved for later LS-11 evidence work. Clean local validation before commit: `CI=true corepack pnpm vitest run tests/e2e/self-hosted/artifact-upload.test.ts` (1 file / 1 test), `CI=true corepack pnpm vitest run tests/conformance/runner-protocol tests/contract/runner-spool tests/contract/artifact-fs tests/contract/artifact-s3 tests/contract/postgres/artifact-upload.test.ts tests/unit/runner/trace-upload-pump.test.ts` (13 files / 156 tests), `CI=true corepack pnpm typecheck`, and `git diff --check`. No PR created and no resolved/final PR evidence added; fresh review remains required because a test file changed after clean review.

- final: reviewed code/test head `abc350bfe86678dbb87c80fe4ee8e8cc47dffeaf`. Complete-matrix review reported no core blockers: `Q:/Qualigence/.pi-subagents/artifacts/outputs/09cc36c3-4bdb-497c-b834-81f4bc0855f1/ticket11-review5/standards.md` and `Q:/Qualigence/.pi-subagents/artifacts/outputs/09cc36c3-4bdb-497c-b834-81f4bc0855f1/ticket11-review5/spec.md`. Final verification passed: `CI=true corepack pnpm vitest run tests/e2e/self-hosted/artifact-upload.test.ts` (1 file / 1 test), `CI=true corepack pnpm vitest run tests/conformance/runner-protocol tests/contract/runner-spool tests/contract/artifact-fs tests/contract/artifact-s3 tests/contract/postgres/artifact-upload.test.ts tests/unit/runner/trace-upload-pump.test.ts` (13 files / 156 tests), `CI=true corepack pnpm typecheck`, and `git diff --check`. Pull request: `https://github.com/ljie-PI/Qualigence/pull/115`.

## Answer

Implemented resumable Artifact upload for Runner Protocol. Runner can register tenant/project/Run-bound Artifact manifests, upload fixed 256 KiB chunks resumably, receive missing ranges, and obtain final ACK only after durable byte/hash/object/manifest verification. Trace ingestion rejects unacknowledged Artifact references, lost-owner replay can finish only previously registered manifests, Runner Spool persists upload progress/leases and replays Artifacts before Trace after reconnect/restart, and the post-review self-hosted E2E verifies PostgreSQL + MinIO/S3 upload/replay behavior.

Pull request: `https://github.com/ljie-PI/Qualigence/pull/115`

Reviewed code/test head: `abc350bfe86678dbb87c80fe4ee8e8cc47dffeaf`

Final verification: focused Ticket 11 Gate, self-hosted Artifact upload E2E, `corepack pnpm typecheck`, and `git diff --check` passed.
