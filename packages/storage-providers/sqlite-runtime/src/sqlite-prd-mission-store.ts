  ...
  async scheduleMission(input: ScheduleMissionInput): Promise<ScheduledMission> {
    ...
    return runInImmediateTransaction(this.runtime, async () => {
      ...
      for (const scheduled of jobs) {
        ...
        await this.write(db.insertInto("execution_runs").values({ run_id: scheduled.job.runId, job_id: scheduled.job.jobId, target_kind: targetKind, objective: scheduled.job.objective, status: "running", next_sequence_number: 1, created_at: input.scheduledAt, completed_at: null, error_code: null }).execute());
