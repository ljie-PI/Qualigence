...
export class SqliteMissionSchedulingRepository implements MissionSchedulingRepository {
  ...
  async schedule(input: Parameters<MissionSchedulingRepository["schedule"]>[0]): Promise<ScheduledMission> {
    ...
    return runInImmediateTransaction(this.runtime, async () => {
      ...
      const plan = await db.selectFrom("test_plan_version_revisions").selectAll().where("plan_id", "=", input.mission.planId).where("version", "=", input.mission.dispatch.binding?.planVersion ?? -1).executeTakeFirst();
      ...
      await this.write(db.updateTable("missions").set({ status: "running" }).where("mission_id", "=", input.command.missionId).where("revision", "=", input.mission.missionRevision).where("status", "=", "approved").where("compiled_hash", "=", input.mission.compiledHash).executeTakeFirst(), "MissionVersionConflict");
      await this.write(db.updateTable("mission_scheduling_heads").set({ version: result.missionVersion }).where("mission_id", "=", input.command.missionId).where("mission_revision", "=", input.mission.missionRevision).where("version", "=", input.command.expectedVersion).where("compiled_hash", "=", input.mission.compiledHash).executeTakeFirst(), "MissionVersionConflict");
  ...
  private async write<T>(operation: Promise<T>, conflict?: "MissionVersionConflict"): Promise<T> {
    const result = await operation;
    if (conflict !== undefined && affected(result) !== 1) throw new MissionSchedulingError(conflict, "Mission changed during scheduling");
    this.writes += 1;
...
function assertTarget(target: { project_id: string; runner_id: string; snapshot_hash: string } | undefined, input: SchedulingInput): void {
  ...
  if (target === undefined || binding === undefined || target.project_id !== input.mission.projectId || target.runner_id !== binding.runnerId || target.snapshot_hash !== binding.targetSnapshotHash) {
