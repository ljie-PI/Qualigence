...
export class PostgresMissionSchedulingRepository implements MissionSchedulingRepository {
  ...
  async schedule(input: Parameters<MissionSchedulingRepository["schedule"]>[0]): Promise<ScheduledMission> {
    ...
    const plan = await this.db.selectFrom("test_plan_version_revisions").selectAll().where("tenant_id", "=", this.tenantId).where("plan_id", "=", input.mission.planId).where("version", "=", binding?.planVersion ?? -1).executeTakeFirst();
    ...
    await this.write(this.db.updateTable("missions").set({ status: "running" }).where("tenant_id", "=", this.tenantId).where("mission_id", "=", input.command.missionId).where("revision", "=", input.mission.missionRevision).where("status", "=", "approved").where("compiled_hash", "=", input.mission.compiledHash).executeTakeFirst(), true);
  ...
  private async write<T>(operation: Promise<T>, cas = false): Promise<T> {
    const result = await operation;
    if (cas && Number((result as { numUpdatedRows?: bigint }).numUpdatedRows ?? 0) !== 1) throw new MissionSchedulingError("MissionVersionConflict", "Mission changed during scheduling");
    this.writes += 1;
...
function assertPlan(plan: { status: string; plan_json: string } | undefined, currentVersion: number | undefined, input: SchedulingInput): void {
  const expectedVersion = input.mission.dispatch.binding?.planVersion;
...
function assertTarget(target: { project_id: string; runner_id: string; snapshot_hash: string } | undefined, input: SchedulingInput): void {
  const binding = input.mission.dispatch.binding;
  if (target === undefined || binding === undefined || target.project_id !== input.mission.projectId || target.runner_id !== binding.runnerId || target.snapshot_hash !== binding.targetSnapshotHash) throw new MissionSchedulingError("MissionHashConflict", "Target revision provenance changed");
}
