...
export class PostgresProjectTargetRepository implements ProjectTargetRepository {
  ...
  async saveRevision(input: SaveTargetRevisionInput): Promise<TargetRevision> {
    ...
    if (head !== undefined && head.project_id !== input.revision.projectId) throw new ProjectTargetError("InvalidTargetConfiguration", "Target revision provenance is inconsistent");
    if (head === undefined) {
      const inserted = await this.db.insertInto("project_targets").values({ tenant_id: this.tenantId, target_id: input.revision.targetId, project_id: input.revision.projectId, current_version: input.revision.version, created_at: input.createdAt, updated_at: input.createdAt }).onConflict((oc) => oc.columns(["tenant_id", "target_id"]).doNothing()).executeTakeFirst();
      if (Number(inserted.numInsertedOrUpdatedRows) !== 1) throw await this.versionConflict(input.revision.targetId);
    }
    else {
      ...
      if (Number(result.numUpdatedRows) !== 1) throw await this.versionConflict(input.revision.targetId);
    }
  ...
  private async versionConflict(targetId: string): Promise<ProjectTargetError> { const authoritative = await this.db.selectFrom("project_targets").select("current_version").where("tenant_id", "=", this.tenantId).where("target_id", "=", targetId).executeTakeFirst(); return new ProjectTargetError("TargetVersionConflict", "Target version conflict", { currentVersion: authoritative?.current_version ?? 0 }); }
}
...
export class PostgresTestPlanRepository implements TestPlanRepository {
  ...
  async saveDraft(input: SaveDraftTestPlanInput): Promise<TestPlanRevision> { const replay = await this.db.selectFrom("test_plan_version_revisions").select("plan_json").where("tenant_id", "=", this.tenantId).where("idempotency_key", "=", input.idempotencyKey).executeTakeFirst(); if (replay !== undefined) { const plan = parsePlan(replay.plan_json); if (JSON.stringify(plan) !== JSON.stringify(input.plan)) throw planStoreError("IdempotencyConflict", plan.version); return plan; } const document = await this.getPrdDocument(input.plan.prdId, input.plan.prdRevision); if (document === undefined || document.projectId !== input.plan.projectId) throw planStoreError("IdempotencyConflict", 0); await this.db.insertInto("test_plan_heads").values({ tenant_id: this.tenantId, plan_id: input.plan.planId, project_id: input.plan.projectId, current_version: input.plan.version, created_at: input.createdAt, updated_at: input.createdAt }).execute(); await this.db.insertInto("test_plan_version_revisions").values(planRow(this.tenantId, input.plan, input.idempotencyKey, input.createdAt)).execute(); return input.plan; }
  async approve(input: ApproveStoredTestPlanInput): Promise<TestPlanRevision> { const replay = await this.db.selectFrom("test_plan_version_revisions").select("plan_json").where("tenant_id", "=", this.tenantId).where("idempotency_key", "=", input.idempotencyKey).executeTakeFirst(); if (replay !== undefined) { const plan = parsePlan(replay.plan_json); if (plan.planId !== input.planId || plan.version !== input.expectedVersion + 1 || plan.approval?.reviewerId !== input.reviewerId) throw planStoreError("IdempotencyConflict", plan.version); return plan; } const current = await this.get(input.planId); if (current === undefined) throw new Error("PlanNotFound"); if (current.status === "approved") throw planStoreError("PlanVersionConflict", current.version); const result = approveTestPlan(current, input, input.clock); if (!result.ok) throw planStoreError(result.error.code, current.version); const approved = result.value; const at = approved.approval?.approvedAt ?? input.clock.now(); const updated = await this.db.updateTable("test_plan_heads").set({ current_version: approved.version, updated_at: at }).where("tenant_id", "=", this.tenantId).where("plan_id", "=", input.planId).where("current_version", "=", input.expectedVersion).executeTakeFirst(); if (Number(updated.numUpdatedRows) !== 1) { const actual = await this.db.selectFrom("test_plan_heads").select("current_version").where("tenant_id", "=", this.tenantId).where("plan_id", "=", input.planId).executeTakeFirst(); throw planStoreError("PlanVersionConflict", actual?.current_version ?? current.version); } await this.db.insertInto("test_plan_version_revisions").values(planRow(this.tenantId, approved, input.idempotencyKey, at)).execute(); return approved; }
  ...
  async getPrdDocument(prdId: string, revision: number): Promise<PrdDocument | undefined> { const row = await this.db.selectFrom("prd_documents").selectAll().where("tenant_id", "=", this.tenantId).where("prd_id", "=", prdId).where("revision", "=", revision).executeTakeFirst(); return row === undefined ? undefined : prdFromRow(row); }
}
