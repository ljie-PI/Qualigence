...
- `LocalRunIntakeStore` exposes only bounded authority operations: `create`, `pendingDispatches(limit)`, `beginOffer(runId, expectedAttempt, at)`, `markOffered(runId, attempt, at)`, `markOfferOutcomeUnknown(runId, attempt, code, at)`, `quarantineInterruptedDispatches(limit, at)`, `run(runId)`, `pendingCompletions(limit)`, and `applyCompletion(runId, completion, completedAt)`. Selectors validate a positive safe bounded limit and use stable `updated_at, created_at, run_id` order. Every transition is expected-state/attempt CAS; `applyCompletion` atomically updates the existing Run terminal fields and completion state. Evidence remains responsible only for Trace/Finding/Artifact reference reads used by GET/readiness. No method creates recovery intent, claims a lost lease, or creates a child Run.
...
- Every created intake begins with durable completion state `awaiting`, so no sink failure can erase retry intent. The Local sink point-reads authoritative `completionRecord(runId)`, validates identity/equality, maps it to existing Run terminal statuses, and calls atomic `applyCompletion`; sink/apply failure leaves the Run nonterminal and `awaiting`, while duplicate apply is idempotent. `LocalRunCoordinator` owns one retained reconciliation promise under Core lifecycle: it performs one serialized bounded startup pass before ready, then repeats bounded passes while live. Config requires positive safe integers with exact defaults/maxima: `completionReconciliationPollIntervalMs` default `250`, maximum `60_000`; `completionReconciliationFailureBackoffMs` default `1_000`, maximum `300_000`; and `completionReconciliationBatchSize` default `64`, maximum `256`. Launcher config passes those non-secret values to Core; Core validates them again. An empty/successful pass waits the poll interval, a failed pass records unhealthy reconciliation and waits the failure backoff, and no pass overlaps another dispatch/reconciliation mutation. Each pass selects `offered | offer_outcome_unknown` + `awaiting`, reads `completionRecord(runId)`, and retries eventual apply. An authoritative completion resolves an unknown offer outcome without re-offer; no completion leaves it quarantined. A mismatched authoritative record becomes `integrity_blocked`. Unmarked and `pending_runner` Runs are never inferred terminal; startup first quarantines every interrupted `dispatching` row before reconciliation. Core shutdown/quiesce aborts the loop's delay, prevents a new pass, and awaits the retained promise before closing SQLite; no fire-and-forget retry survives shutdown.
...
Add issuer/session/HTTP/coordinator/readiness/bootstrap tests first. Prove strict schemas/auth kinds; exact unpadded 43-character base64url external encoding; exact `201 { sessionToken, expiresAt }`; body/query/content-type rejection; exact one-frame fd-3 codec, truncation/trailing/timeout/zeroization; deterministic isolated-only issuance; atomic Run+marker rollback; one dispatcher claim; known pre-offer retry; post-start uncertainty quarantine; no automatic lease recovery; completion sink ordering/failure/idempotency; serialized startup/live reconciliation with poll/failure backoff and eventual terminalization; abort/await shutdown; and exact Runner identity/capability readiness. Extend all Runner-control adapters/helper/contracts for `completionRecord` in the same RED change. Extend existing `ProcessSupervisor` tests for unchanged foreground handling plus detached private-child acknowledgement, marker wakeup, quiesce, reverse-order stop, failure escalation, and cleanup; do not create a parallel supervisor test surface.
...
Implement the shared fixed binary frame and thin Launcher/Core handoff modules, exact unpadded base64url Bearers, then hash-only bootstrap/session/quiesce authorization. Bind strict Fastify Local routes to exact `127.0.0.1`; session has no body/query/content-type and returns only exact status/body. Validate `{ targetUrl, objective }` before issuer invocation, create IDs/project/policy deterministically, atomically persist Run+marker, and immediately return durable `202 pending_runner`; request handling never waits for Runner acceptance.

- [ ] **Step 4: Implement serialized dispatch and authoritative completion**

Run one abortable dispatcher over the runner-control-owned intake store. Retry only states/failures known not to have started `offer`; quarantine all post-start uncertainty. Extend the existing connection object with its authenticated capability snapshot rather than adding a registry. Return Task 10 completion disposition, invoke `RunCompletionSink` only after authority commits, persist retryable reconciliation state, and retain the configured serialized startup/live reconciliation loop through `completionRecord`. Abort and await both loops before storage shutdown. Do not call or extend automatic lease recovery APIs.

- [ ] **Step 5: Implement truthful readiness and Launcher supervision**

Compose SQLite/Run/Trace/manifest and Artifact-byte probes, listener flags, live reconciliation health, and configured Runner identity/capability into one readiness service. Both modes use the existing `ProcessSupervisor`: migration preflight; generate two credentials; start Core with one fd-3 frame; wait internal ready; start Runner; wait final ready. Foreground retains the supervisor and existing signal lifetime. Detached mode starts only the supervisor's private child adapter, sends the supervisor credential once over inherited Node IPC, waits acknowledgement, atomically writes runtime state, prints the unpadded bootstrap once, zeroes retained buffers, detaches, and exits. Detached stop uses only the non-secret marker poll, authenticated quiesce, existing Runner-before-Core termination, cleanup, and runtime-state removal last. Add no public lifecycle interface, second supervisor module, `DataDirLock` implementation, or handoff.
