...
Task 10
    → Task 11 design dossier only (non-production; no RED, implementation, Gate, commit, or dispatch activation)
...
**Policy sequencing:** The only parallel Task 11 work authorized while Task 15 is in progress is a non-production design dossier. It may not edit Task 11 production or test source, create its RED test, run its Task Gate, make its Task commit, or activate any Local dispatch. Task 11 implementation, its RED test, its Gate and commit, and all Local production dispatch activation begin only after Task 15 merges with required Job policy and no allow-all composition.
...
`SqliteRunnerControlStore` is fail-closed by default. Its only exception is an explicit `SqliteRunnerControlStoreOptions.legacyM1LocalRecovery` supplied for a Local SQLite migration/recovery. Export that option from `packages/storage-providers/sqlite-runtime/src/index.ts`; it is not a boolean and has no default. `apps/core-daemon/src/config.ts` may load it only from an explicitly named `CORE_LEGACY_M1_LOCAL_RECOVERY_MANIFEST` file when `CORE_DEPLOYMENT_MODE=local` and `CORE_HOST` is loopback; any manifest in another mode or on a non-loopback host is an invalid configuration. Its required `legacy-m1-local-recovery/v1` records each exact `{ jobId, runId, canonicalJobSha256, policy }`. `apps/core-daemon/src/main.ts` passes that option only when this Local manifest is present; absence remains strict rejection. The manifest policy must be exactly `policyId: "legacy-m1-local"`, `environment: "isolated_test"`, `allowedActionKinds: ["click"]`, `maximumRisk: "Normal"`, `explorationAllowed: false`, one allowed origin equal to the persisted Web target origin, and valid `issuedAt`/`expiresAt` with `issuedAt < expiresAt`. The store upcasts only when the parsed policyless Job's job/run IDs and `canonicalPayloadHash(policylessJob)` exactly match one manifest record. It must reject duplicate records, a hash/identity/origin mismatch, any other policy shape, and every policyless row without that exact match as `PolicyMissing`. No environment switch, deployment default, wildcard, target-derived policy, or arbitrary record may activate this path.

`tests/contract/runner-control/sqlite-runner-control-store.test.ts` must seed raw policyless `execution_leases.job_json` and prove default rejection, then prove only a fully matching manifest record yields the exact configured `legacy-m1-local` snapshot. `tests/contract/runner-control/postgres-runner-control-store.test.ts` must seed the same raw shape and prove unconditional `PolicyMissing`; `tests/unit/core-daemon/config.test.ts` covers manifest absence and malformed/duplicate entries; `tests/component/core-runner/core-composition.test.ts` proves normal Core composition has no recovery manifest and cannot offer a policyless persisted Job; and `tests/component/core-runner/disconnect-recovery.test.ts` proves `RunOwnershipService.recoveryJob` only copies a prevalidated stored policy, never constructs `legacy-m1-local`. These are storage reads only: wire payloads, newly persisted Jobs, and unknown/recovered production Jobs never receive an upcast.
...
```bash
git add packages/core-modules/runner-control/src/runner-control-store.ts packages/core-modules/runner-control/src/index.ts packages/execution-application/src/index.ts apps/core-daemon/src/runner/remote-runner-target.ts apps/core-daemon/src/index.ts apps/core-daemon/src/config.ts apps/core-daemon/src/main.ts apps/core-daemon/package.json apps/core-daemon/tsconfig.json packages/storage-providers/sqlite-runtime/src/index.ts pnpm-lock.yaml tests/contract/runner-control/sqlite-runner-control-store.test.ts tests/contract/runner-control/postgres-runner-control-store.test.ts tests/unit/core-daemon/config.test.ts tests/component/core-runner/core-composition.test.ts
