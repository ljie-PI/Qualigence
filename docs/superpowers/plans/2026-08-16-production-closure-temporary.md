...
- `LocalRunIntakeStore` exposes only bounded authority operations: `create`, `pendingDispatches(limit)`, `beginOffer`, `markOffered`, `markOfferOutcomeUnknown`, `quarantineInterruptedDispatches`, `run`, `pendingCompletions({ now, limit })`, `recordCompletionAttempt({ runId, expectedAttempt, attemptedAt, nextAttemptAt })`, `recordCompletionFailure({ runId, attempt, safeErrorCode, failedAt, nextAttemptAt })`, `applyCompletion({ runId, attempt, jobId, jobSha256, completion, completedAt })`, and `markIntegrityBlocked({ runId, expectedAttempt, safeErrorCode, blockedAt })`. Selectors validate canonical database-comparable `now` and positive safe bounded limit, return only `offered | offer_outcome_unknown` + `awaiting` rows with `completion_next_attempt_at <= now`, and use stable `completion_next_attempt_at, updated_at, run_id` order. Dispatch transitions retain their expected-state/attempt CAS. Evidence remains responsible only for Trace/Finding/Artifact reference reads used by GET/readiness. No method creates recovery intent, claims a lost lease, or creates a child Run.
- Migration 007 completion columns are exact: `completion_state` constrained to `awaiting | applied | integrity_blocked`; `completion_attempt` nonnegative integer default `0`; nullable `completion_last_attempt_at`; nonnull `completion_next_attempt_at` initialized to `created_at`; nullable `completion_error_code`; nullable lowercase-hex `completion_sha256`; nullable `completion_applied_at`; and nullable `completion_blocked_at`. `recordCompletionAttempt` CASes `awaiting` plus `expectedAttempt`, increments to `attempt = expectedAttempt + 1`, sets `completion_last_attempt_at = attemptedAt`, clears `completion_error_code`, and advances `completion_next_attempt_at` to checked `attemptedAt + completionReconciliationFailureBackoffMs`; that due-time reservation guarantees retry after a crash mid-attempt. `recordCompletionFailure` CASes the exact current attempt, stores only `CompletionPending | CompletionAuthorityUnavailable | CompletionApplyFailed`, and sets a canonical `nextAttemptAt` no earlier than `failedAt` and exactly `failedAt + configured failure backoff`; raw error/message/stack is never persisted. Both reject stale attempt/time input without mutation.
- `applyCompletion` CASes exact `awaiting` + attempt and revalidates stored jobId/canonical Job SHA-256, completion jobId/runId, and canonical completion hash. It atomically writes existing Run terminal status/completedAt/errorCode, stores `completion_sha256`, and changes marker to `applied`. Its result is exactly `applied | duplicate | stale | identity_mismatch | completion_conflict`: `duplicate` requires already-`applied`, identical job/hash, canonical completion hash, terminal status/completedAt/errorCode, and makes no write; an altered replay is never duplicate. Coordinator maps either mismatch result to `markIntegrityBlocked`, which CASes exact `awaiting` + expected attempt to `integrity_blocked`, stores only `CompletionIdentityMismatch` or `CompletionConflict`, and leaves the Run nonterminal. Store/provider contracts prove every transition, concurrent/stale attempts, crash-due retry, safe-error persistence, exact duplicate, mismatch block, close/reopen, schema constraints, and no internal state in the public projection.
...
- Every created intake begins with durable completion state `awaiting`, so no sink failure can erase retry intent. The Local sink and loop use the same serialized pass: claim with `recordCompletionAttempt`, point-read authoritative `completionRecord(runId)`, validate identity/hash, then call `applyCompletion`; any transient read/apply error calls `recordCompletionFailure`, leaving the Run nonterminal/`awaiting` and due for live retry. `LocalRunCoordinator` owns one retained reconciliation promise under Core lifecycle: one bounded startup pass before ready, then repeated bounded live passes. Config requires positive safe integers with exact defaults/maxima: `completionReconciliationPollIntervalMs` default `250`, maximum `60_000`; `completionReconciliationFailureBackoffMs` default `1_000`, maximum `300_000`; and `completionReconciliationBatchSize` default `64`, maximum `256`. Launcher passes those non-secret values to Core; Core validates them again. Empty/successful pass waits poll interval; failed pass records each claimed row's safe failure/due time, marks reconciliation unhealthy, and waits failure backoff; no pass overlaps dispatch/reconciliation mutation. An authoritative completion eventually terminalizes `offered` or resolves `offer_outcome_unknown` without re-offer; no completion leaves unknown quarantined. Job/hash/completion mismatch calls `markIntegrityBlocked` and never retries or terminalizes. Core shutdown/quiesce aborts delay, prevents a new pass, and awaits the retained promise before SQLite close; no fire-and-forget retry survives shutdown.
