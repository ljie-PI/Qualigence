...
2026-08-01（PR-18 / LS-10）：在 `feat/ls-10-integration-gate` 分支完成 LS-10 最后一环——集成 Gate（Task 5 剩余部分），接续 PR-16（调查/审阅领域与持久化、迁移 005）与 PR-17（Evidence Capsule 密码学：`packages/core-modules/evidence/src/capsule/**`、`packages/runner-components/evidence-capsule/**`、`packages/storage-providers/kms-self-hosted/**`）。至此 LS-10 全部实现。新增 `packages/storage-providers/sqlite-runtime/src/sqlite-evidence-capsule-store.ts`（`SqliteEvidenceCapsuleStore`）：**纯增量 store 代码，不新增/改动任何迁移**，只读写 PR-16 迁移 005 既有的 Evidence Capsule 元数据表（`evidence_encryption_profiles`、`evidence_capsule_manifests`、`evidence_capsule_entries`、`evidence_key_rotations`、`evidence_local_only_records`、`evidence_audit_events`）。该 store 实现 `EvidenceAuditSink`，可直接作为 `EvidenceEnvelopeEncryptor` 的审计汇——`wrap`/`unwrap` 审计随 manifest 一并落库；`saveRemoteCapsule` 事务性持久化 profile + manifest revision + entry 元数据 + ciphertext blob；`loadCapsule` 默认取最新 revision 并重建 `EncryptedCapsule`；`rotateKey` 追加不可变新 revision 并保留父 revision/actor/reason/old-new key id 审计；`saveLocalOnly` 写入独立表，`listRemoteUploads` 只查 manifest 表故 local-only 记录绝不进入远端上传查询。**revoke-before-delete 硬不变量**在 store 层强制：`revokeCapsule` 先调 KMS `revoke` 使解包许可失效、成功后才把 manifest 置 `revoked` 并写成功审计，KMS revoke 失败则保留 ciphertext + `active` 态并写 `failed` 审计（绝不出现“数据已删但解包许可状态未知”）；`deleteCiphertext` 只要仍有 `active` revision 就抛 `EvidenceLifecycleError('EvidenceRevokeRequiredBeforeDelete')`，故删除绝不先于撤销；`expireCapsule` 按“先 revoke 后 delete”编排。集成路径全部由既有公共导出组合，未修改 PR-16/PR-17 拥有的 `core-modules/{investigation,intelligence,review}`、`evidence/capsule`、`runner-components/evidence-capsule`、`kms-self-hosted` 任何 `src`。测试：`tests/contract/sqlite/evidence-capsule-store.test.ts`（重开库字节往返、entry/审计落库、未撤销即删除被拒、revoke→delete 顺序、revoke 失败保留 ciphertext、密钥轮换新 revision、local-only 隔离）；`tests/component/investigation/offline-capsule-restoration.test.ts`（真实 Chromium 复现→确认 BugEpisode→用真实截图/语义图 bytes 经真实 KMS 加密的 Capsule→`SqliteEvidenceCapsuleStore` 持久化→**关闭并重开 DB 文件 + 删除 Runner 本地 artifact 目录**→仅凭持久化 Capsule + KMS 离线解密并逐字节比对还原真实截图/语义图 bytes；非复现 Finding 预算耗尽→持久化 Human ReviewTask 且无远端 Capsule 上传；持久化 Capsule 上 revoke-before-delete 端到端）。验证证据：`pnpm build`、`pnpm test`（564 passed / 1 skipped，96 文件，较基线 554 新增 10 用例——7 store 契约 + 3 集成，无既有回归）、`pnpm typecheck`、`pnpm smoke:node-imports`、`git --no-pager diff --check` 均通过。偏差：Task 5 计划文件清单中的迁移 005 与 `sqlite-investigation-store.ts`/`sqlite-review-store.ts` 已由 PR-16 交付，本 PR 仅补 `sqlite-evidence-capsule-store.ts` 与集成/离线测试，未触碰迁移（`SUPPORTED_SCHEMA_VERSION` 保持 5）；离线还原测试将 `InMemoryTestKms` 实例视为跨 DB 重开持续存在的“KMS 服务”（离线的是 Runner 与本地 DB，而非 KMS），符合设计“仅凭持久化 Capsule + KMS access 受控调查”的语义；调查聚合本身的持久化交接与 local-only schema 隔离已由 PR-16 的 `tests/component/investigation/offline-capsule-flow.test.ts` 覆盖，本 PR 的集成测试聚焦新增的真实加密 Capsule 端到端离线还原与生命周期。
...
| LS-10 | M2 | Reproduction、Bug Episode、Human Review、Evidence Capsule | `implemented` | `docs/superpowers/specs/2026-08-01-ls-10-m2-investigation-review-evidence-design.md` | `docs/superpowers/plans/2026-08-01-ls-10-m2-investigation-review-evidence.md` | `packages/core-modules/{investigation,intelligence,review}/**`、`packages/core-modules/evidence/src/capsule/**`（PR-17）、`packages/runner-components/{model-agent/src/investigation-agent.ts,evidence-capsule/**}`、`packages/storage-providers/{kms-self-hosted/**,sqlite-runtime/src/{migrations/005-investigation-review.ts,sqlite-investigation-store.ts,sqlite-review-store.ts,sqlite-intelligence-store.ts,sqlite-evidence-capsule-store.ts}}`（PR-18 集成 Gate）、`tests/component/investigation/offline-capsule-restoration.test.ts`、`tests/contract/sqlite/evidence-capsule-store.test.ts` |
